        ...
        if kind=='full':
            ...
            if len(feasible)==1: weights[feasible[0]]=delta
                ...
                for j in feasible:
                    ...
                    weights[j]=delta*(F(1,4) if neighbors==2 else F(3,4))
            elif len(feasible)==5: weights=[delta*F(5,12)]*5
    ...
    for x in range(32):
        ...
        for i in range(5):
            ...
            for event in range(64):
                diff=sum(P[x][j]-P[y][j] for j in range(6) if event&(1<<j))
                assert diff<=delta
...
def run():
    ...
    dest.write_text(json.dumps({'mechanisms':len(rows),'event_inequalities':sum(r['events_checked'] for r in rows),'rows':rows},indent=2))
